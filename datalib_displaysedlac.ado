* v.0.4.           <22Nov2013>  <Andres Castaneda>


program define datalib_displaysedlac, rclass
...
if ("`country'" == "all") {
	local countryname: dir "${rootdatalib}/sedlac" dirs "*"
}
...
* =========================================================================================================
	...
	tempname t
	tempfile table
	postfile `t' str3 country str20 pais str10 year str10 survey str10 type str4 Project str4 MastVersion str4 AltVersion str6 file using `table', replace

foreach pais of local paiss {
	if (substr("`pais'",1,1) == "_") continue		//	if local pais starts with "_"

	* Find subfolders on Country directory
	local dir1 "${rootdatalib}/sedlac/`country'"  // country dir
	...
	foreach folder of local folders {  // folder like ARG_2006_EPHC
		**local folder = lower("`folder'")
		lstrfun year, regexms(`"`folder'"', `"(^[a-z]+_)([0-9]+-)?([0-9]+)_*"', 3)
		...
		procedure for _base
		==================== */
		local subfolders: dir "`dir1'/`folder'" dirs "*base"
		local type "base"
		if (`"`subfolders'"' == `""') {
			local mv = "01"
			post `t' ("`country'") ("`pais'") (`"`year'"') ("`survey'") ("`type'") (".") ("`mv'") (".") ("NO")
		}
		else {
			foreach subfolder of local subfolders {		// folders like arg_2006_s1_ephc_v01_m_base
				...
				lstrfun pd, regexms("`subfolder'", "(^`country'_`year')([_]?[a-zA-Z]?[0-9]?)(_.*)",2) 	// period
				cap confirm file "`dir1'/`folder'/`subfolder'/data/`subfolder'.dta"
				if (_rc == 0) local exist "YES"
				else local exist "NO"
				post `t' ("`country'") ("`pais'") (`"`year'`pd'"') ("`survey'") ("`type'") (".") ("`mv'") (".") ("`exist'")
			}
		}	//  end of olse
		
		/* ======================
		procedure for _sedlac*
		====================== */
		local subfolders: dir "`dir1'/`folder'" dirs "*sedlac*"
		foreach subfolder of local subfolders {
			...
			foreach type in sedlac cedlas {
				local file:	dir "`dir1'/`folder'/`subfolder'/Data/Harmonized" files "*_`type'_`pr'.dta"
				if (`"`file'"' == "") local exist "NO"
				else local exist "YES"
				post `t' ("`country'") ("`pais'") (`"`year'`pd'"') ("`survey'") ("`type'") ("`pr'") ("`mv'") ("`av'") ("`exist'")
			} 	// end of loop for sedlac and cedlas
		}		// end of subfolder loop in SEDLAC section
	...
	label var Project "Sedlac Project"
	...
	label var type "Type of SEDLAC data"
	...
	** return values
	...
	levelsof year if file == "YES", loc(years) clean
...
if ("`country'" != "all") {
	if ("`typedta'" == "base") {
		...
		tabdisp ys MastVersion if (type == "base" ), cell(file) center
		dis as text "{hline}"
	}
	if ("`typedta'" == "sedlac") {
		dis as text "{hline}" 
		noi dis as text `"{p 4 4 2}{cmd:Inventory of "_sedlac`project'" and "_cedlas`project'" databases for :}"' in y "`pais'" as text `" {p_end}"'
...
v.0.3.           <08Nov2013>  <Andres Castaneda>
